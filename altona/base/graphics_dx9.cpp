...
void InitGFX(sInt flags_,sInt xs_,sInt ys_)
{
  ...
  if(flags2 & sSM_FULLSCREEN)
  {
    ...
    modeex.Width = DXScreenMode.ScreenX;
    modeex.Height = DXScreenMode.ScreenY;
    modeex.RefreshRate = DXScreenMode.Frequency;
    modeex.Format = colormode;
...
void sBeginSaveRTPrivate(const sU8*& data, sS32& pitch, enum sTextureFlags& flags,IDirect3DSurface9* source)
{
  ...
  switch(desc.Format)
  {
    ...
    case D3DFMT_A8B8G8R8:       flags = sTEX_ABGR8888; break;
...
void ConvertFlags(sU32 flags, D3DFORMAT& d3df, D3DPOOL& pool, sInt& usage, sInt& mm, sInt& lflags)
{
  ...
  switch(flags&sTEX_FORMAT)
  {
    ...
    case sTEX_ABGR8888: d3df = D3DFMT_A8B8G8R8;     break;
...
{
  DXErr(DXDev->UpdateTexture(from->Tex2D, Tex2D));
}
