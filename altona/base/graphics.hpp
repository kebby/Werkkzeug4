...
enum sTextureFlags 
{
  ...
  sTEX_ABGR8888     = 43,         // 32bit texture that's RGBA instead of BGRA
...
public:
  sTexture2D();
  sTexture2D(sInt xs,sInt ys,sU32 flags,sInt mipmaps=0,void *shareHandle=0);
  virtual ~sTexture2D();

  void Init(sInt xs, sInt ys, sInt flags,sInt mipmaps=0, sBool force=sFALSE, void *shareHandle = 0);
  ...
  void UpdateFrom(sTexture2D *from);
