...
sBool sShaderCompileDX(const sChar *source,const sChar *profile,const sChar *main,sU8 *&data,sInt &size,sInt flags,sTextBuffer *errors)
{
  ...
  UINT flags1 = 0;
  UINT flags2 = 0;
  if (flags&sSCF_DEBUG) flags1 |= D3DCOMPILE_DEBUG;
  if(flags&sSCF_AVOID_CFLOW) flags1 |= D3DCOMPILE_AVOID_FLOW_CONTROL;
  if(flags&sSCF_PREFER_CFLOW) flags1 |= D3DCOMPILE_PREFER_FLOW_CONTROL;
  if(flags&sSCF_DONT_OPTIMIZE) flags1 |= D3DCOMPILE_SKIP_OPTIMIZATION;
